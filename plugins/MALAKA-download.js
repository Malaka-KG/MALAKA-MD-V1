...
cmd(commandDetails, async (bot, message, options, context) => {
  try {
    const { from, reply, q } = context;
    ...
    await options.react('⬇️');

    // Fetch video details
    const downloadData = await downloadTiktok(q);
...
URL: ${q}
    ...
    const metadata = {
      ...
      externalAdReply: {
        ...
        body: "Download videos and audio from TikTok",
    ...
    const sentMessage = await bot.sendMessage(from, { image: imageDetails, caption: menu, contextInfo: metadata });

    // Handle user response
    bot.ev.on("messages.upsert", async (newMessages) => {
      const userMessage = newMessages.messages[0];

      if (!userMessage.message) return;

      const responseText = userMessage.message.conversation || userMessage.message.extendedTextMessage?.text;

      if (responseText === '1') {
        // Send SD Quality Video
        await bot.sendMessage(from, { video: { url: downloadData.result.dl_link.download_mp4_1 }, caption: "SD Quality Video" });
      } else if (responseText === '2') {
        // Send HD Quality Video
        await bot.sendMessage(from, { video: { url: downloadData.result.dl_link.download_mp4_2 }, caption: "HD Quality Video" });
      } else if (responseText === '3') {
        // Send Audio File
        await bot.sendMessage(from, { audio: { url: downloadData.result.dl_link.download_mp3 }, mimetype: "audio/mpeg" });
      }
    });
  } catch (error) {
    console.error(error);
