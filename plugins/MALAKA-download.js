...
cmd(config, async (bot, message, chat, {
  from,
  prefix,
  quoted,
  body,
  ...
  try {
    ...
    if (viewOnceMessage) {
      if (viewOnceMessage.message.imageMessage) {
        ...
        const savedMediaPath = await bot.downloadAndSaveMediaMessage(viewOnceMessage.message.imageMessage);
        const media = { url: savedMediaPath };
        const payload = { image: media, caption };
        return bot.sendMessage(from, payload);
      }

      if (viewOnceMessage.message.videoMessage) {
        const caption = viewOnceMessage.message.videoMessage.caption;
        const savedMediaPath = await bot.downloadAndSaveMediaMessage(viewOnceMessage.message.videoMessage);
        const media = { url: savedMediaPath };
        const payload = { video: media, caption };
        return bot.sendMessage(from, payload);
      }
    }
    ...
    if (quoted.mtype === "viewOnceMessage") {
      console.log("Handling ViewOnce message.");
      if (quoted.message.imageMessage) {
        const caption = quoted.message.imageMessage.caption;
        const savedMediaPath = await bot.downloadAndSaveMediaMessage(quoted.message.imageMessage);
        const media = { url: savedMediaPath };
        const payload = { image: media, caption };
        return bot.sendMessage(from, payload);
      } else if (quoted.message.videoMessage) {
        const caption = quoted.message.videoMessage.caption;
        const savedMediaPath = await bot.downloadAndSaveMediaMessage(quoted.message.videoMessage);
        const media = { url: savedMediaPath };
        const payload = { video: media, caption };
        return bot.sendMessage(from, payload);
      }
    ...
    const payload = { react: reaction };
    await bot.sendMessage(from, payload);
